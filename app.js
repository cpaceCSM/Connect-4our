...
function clickBox(){ 
    ...
    alert("Sorry! You must place your tokens in an empty space starting at the bottom of the board.")
    } 
    }
...
function checkWon(){
    ...
    if(square.every(q=>squares[q].classList.contains("player-one"))){
      setTimeout(() =>alert("Brown Wins!"), 200)
      setTimeout(() =>restart.style.display="flex", 500)
      }else if(square.every(q=>squares[q].classList.contains("player-two"))){
      setTimeout(() =>alert("Sienna Wins!"), 200)
      setTimeout(() =>restart.style.display="flex", 500)
      if(box===42){ 
        setTimeout(()=>alert("boxes filled"),300)
        setTimeout(()=>restart.style.display="flex",500) 
